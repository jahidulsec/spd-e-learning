...
const getMulti = async (queries: LeaderboardQueryInputTypes) => {
  ...
  ranked AS (
    SELECT *,
          ...
          ${
            view === "quiz"
              ? "ORDER BY (quiz_score) DESC, is_completed desc, duration_s asc, full_name asc"
              : view === "e_detailing"
                ? "ORDER BY (e_score) DESC, full_name asc"
